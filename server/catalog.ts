...
export async function pathsIn(cwd: string, versions?: Map<string, string>): Promise<string[]> {
  const paths: string[] = [];
  const read = async (dir: string, limit: number) => {
    if (versions) versions.set(dir, await directoryVersion(dir));
    return directoryEntries(dir, limit);
  };
  const skip = new Set(['node_modules', '.git', '.venv', 'venv', 'dist', '.cache']);
  const entries = await read(cwd, 10000);
  ...
  for (const dir of entries) {
    ...
    const children = await read(path.join(cwd, dir.name), 4000 - paths.length);
